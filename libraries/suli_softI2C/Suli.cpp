 ...
 * - len: data lenght
 */
uint8 suli_i2c_write(uint8 dev_addr, uint8 *data, uint8 len)
{
    dev_addr = dev_addr>>1;                                         
    
    Wire1.beginTransmission(dev_addr);          // start 
    for(int i=0; i<len; i++)
    {
        Wire1.write(data[i]);                   // send a byte
    }
 ...
 * return
 */
uint8 suli_i2c_read(uint8 dev_addr, uint8 *buff, uint8 len)
{
    dev_addr = dev_addr>>1;
    Wire1.requestFrom(dev_addr, len);
    
    int sum_len = 0;
    while(Wire1.available())
    {
        buff[sum_len++] = Wire1.read();
    }
    return sum_len;
}
