...
{
    uint8 dta[2] = {0x00};
    suli_i2c_write(DS1307_I2C_ADDRESS, dta, 1);
    
    suli_i2c_read(DS1307_I2C_ADDRESS, dta, 1);
    
    dta[1] = dta[0] & 0x7f;
    dta[0] = 0x00;
    suli_i2c_write(DS1307_I2C_ADDRESS, dta, 2);
}
...
void rtc_ds1307_get_time(struct __time * time_get)
{
    uint8 dta[1] = {0x00};
    suli_i2c_write(DS1307_I2C_ADDRESS, dta, 1);
    suli_i2c_read(DS1307_I2C_ADDRESS, time_get->data, 7);
...
void rtc_ds1307_set_time(struct __time t)
{
    ...
    suli_i2c_write(DS1307_I2C_ADDRESS, dta, 8);
}