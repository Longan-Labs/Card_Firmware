...
boolean VL53L1X::begin(uint8_t deviceAddress)
{
  ...
  //Wires = &wirePort; //Grab which port the user wants us to use
...
void VL53L1X::startMeasurement(uint8_t offset)
{
  ...
  while (leftToSend > 0)
  {
    ...
    Wires.beginTransmission(_deviceAddress);

    Wires.write(0); //We're only in lower address space. No MSB needed.
    Wires.write(address);

    for (byte x = 0 ; x < toSend ; x++)
      Wires.write(configBlock[address + x - 1 - offset]);
...
{
  Wires.beginTransmission(_deviceAddress);
  Wires.write(addr >> 8); //MSB
  Wires.write(addr & 0xFF); //LSB
  if (Wires.endTransmission() != 0) //Send a restart command. Do not release bus.
    return (0); //Sensor did not ACK

  Wires.requestFrom((uint8_t)_deviceAddress, (uint8_t)1);
  if (Wires.available())
    return (Wires.read());
...
uint16_t VL53L1X::readRegister16(uint16_t addr)
{
  ...
  Wires.requestFrom((uint8_t)_deviceAddress, (uint8_t)2);
  if (Wires.available())
  {
    uint8_t msb = Wires.read();
    uint8_t lsb = Wires.read();
...
boolean VL53L1X::writeRegister16(uint16_t addr, uint16_t val)
{
  ...
  Wires.write(addr >> 8); //MSB
  Wires.write(addr & 0xFF); //LSB
  Wires.write(val >> 8); //MSB
  Wires.write(val & 0xFF); //LSB
  if (Wires.endTransmission() != 0)
    return (0); //Error: Sensor did not ACK
  return (1); //All done!
}
